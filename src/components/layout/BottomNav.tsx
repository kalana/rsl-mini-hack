        ...
        {navItems.map((item) => {
          ...
          return (
            <Link
              key={item.href}
              href={item.href}
              className={`flex flex-col items-center gap-1 py-3 px-4 transition-colors duration-150 ${
                active
            ...
            >
              {item.icon(active)}
              <span className={`text-[10px] font-medium ${active ? 'text-[var(--color-primary-500)]' : ''}`}>
                {item.label}
              </span>
            </Link>
          );
